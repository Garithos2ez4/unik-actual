...
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h1 class="h3 mb-0 text-gray-800">Publicaciones Falabella Seller Center</h1>
        <div class="d-flex gap-2">
            <form method="GET" class="d-flex gap-2 bg-white p-2 rounded shadow-sm border">
                ...
                @if($search || $filter !== 'all')
                    <a href="{{ route('plataformas.falabella.publicaciones') }}" class="btn btn-outline-secondary btn-sm">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="pe-4 text-end">
                                ...
                                @endphp
                                <a href="{{ route('createpublicacion', [$encryptedId]) . '?' . http_build_query($createParams) }}" class="btn btn-sm btn-outline-success rounded-pill shadow-sm" title="Pre-rellenar Publicación">
        ...
        <div class="card-footer bg-white border-0 py-3">
            <div class="d-flex justify-content-between align-items-center">
                <div class="small text-muted">
                    Mostrando {{ count($products) }} publicaciones (Offset: {{ $offset }})
                </div>
                <div class="btn-group">
                    <a href="{{ route('plataformas.falabella.publicaciones', array_merge(request()->query(), ['offset' => max(0, $offset - $limit)])) }}" 
                       class="btn btn-outline-secondary btn-sm {{ $offset <= 0 ? 'disabled' : '' }}">
                        <i class="bi bi-chevron-left me-1"></i> Anterior
                    </a>
                    <a href="{{ route('plataformas.falabella.publicaciones', array_merge(request()->query(), ['offset' => $offset + $limit])) }}" 
